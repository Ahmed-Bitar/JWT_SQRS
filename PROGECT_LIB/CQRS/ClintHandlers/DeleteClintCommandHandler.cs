...
namespace PROGECT_LIB.CQRS.ClintHandlers
{
    public class DeleteClintCommandHandler: IRequestHandler<DeleteClientCommand, Doctor>
    {
        ...
        public async Task<Doctor> Handle(DeleteClientCommand request, CancellationToken cancellationToken)
        {
            var client = await _context.Doctors.FirstOrDefaultAsync(b => b.Id == request.Id, cancellationToken);
            _context.Doctors.Remove(client);
